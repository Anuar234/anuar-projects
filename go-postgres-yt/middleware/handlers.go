...
func UpdateStock(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Fatalf("Unable to decode th request body. %v", err)
	}

	updatedRows := updateStock(int64(id), stock)

	msg := fmt.Sprintf("Stock updated sucessfully. Total rows/records affected %v", updatedRows)
	res := responce {
		ID: int64(id),
		Message: msg,
	}
...
func DeleteStock(w http.ResponseWriter, r *http.Request){
	...
	msg := fmt.Sprintf("Stock deleted sucessfu;ly. Total rows/records %v", deleteRows)
